...
class KANLastDim(nn.Module):
    """
    Applies a KANLinear to the last dimension of an arbitrary-shaped tensor.
    Keeps device/dtype and works as a drop-in for nn.Linear in most uses.
    """
    def __init__(self, in_features, out_features, **kan_kwargs):
        super().__init__()
        self.kan = KANLinear(in_features, out_features, **kan_kwargs)

    def forward(self, x: torch.Tensor):
        # x: (..., in_features)
        orig_shape = x.shape
        assert x.size(-1) == self.kan.in_features, \
            f"KANLastDim: expected last dim={self.kan.in_features}, got {x.size(-1)}"
        x2 = x.reshape(-1, orig_shape[-1])
        y2 = self.kan(x2)
        y = y2.reshape(*orig_shape[:-1], y2.size(-1))
        return y
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print(f"VSS Block with KAN layers Initialised")
        ########################################################################################
        # self.in_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
        self.in_proj = KANLinear(
    ...
    def _apply_kan_lastdim(self, layer: nn.Module, x_4d: torch.Tensor) -> torch.Tensor:
        # x_4d: (B, H, W, C_in)  ->  out: (B, H, W, C_out)
        B, H, W, C = x_4d.shape
        x2 = x_4d.reshape(B * H * W, C)
        y2 = layer(x2)               # KANLinear forward is (N, Cin) -> (N, Cout)
        y4 = y2.reshape(B, H, W, -1)
        return y4
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # xz = self.in_proj(x)
        xz = self._apply_kan_lastdim(self.in_proj, x)
        ...
        # out = self.out_proj(y)
        out = self._apply_kan_lastdim(self.out_proj, y)
