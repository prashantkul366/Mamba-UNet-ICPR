...
class ConvBlockBHWC(nn.Module):
    """
    2× (Conv3x3 -> Norm -> SiLU) operating on BHWC by permuting to BCHW internally.
    Uses GroupNorm to be layout-agnostic (works well for small batch sizes).
    """
    def __init__(self, dim, groups=8, drop_path=0.0):
        super().__init__()
        self.drop_path = DropPath(drop_path) if drop_path > 0 else nn.Identity()
        self.conv1 = nn.Conv2d(dim, dim, kernel_size=3, padding=1, bias=False)
        self.norm1 = nn.GroupNorm(num_groups=min(groups, dim), num_channels=dim)
        self.act1  = nn.SiLU()
        self.conv2 = nn.Conv2d(dim, dim, kernel_size=3, padding=1, bias=False)
        self.norm2 = nn.GroupNorm(num_groups=min(groups, dim), num_channels=dim)
        self.act2  = nn.SiLU()

    def forward(self, x):  # x: (B, H, W, C)
        B, H, W, C = x.shape
        y = x.permute(0, 3, 1, 2).contiguous()  # to BCHW
        y = self.conv1(y); y = self.norm1(y); y = self.act1(y)
        y = self.conv2(y); y = self.norm2(y); y = self.act2(y)
        y = y.permute(0, 2, 3, 1).contiguous()  # back to BHWC
        return x + self.drop_path(y)            # residual like VSSBlock

class ConvLayer_up(nn.Module):
    """
    Decoder layer: [depth × ConvBlockBHWC] + optional PatchExpand (×2 upsample)
    """
    def __init__(self, dim, depth, drop_path=0.0, upsample=True):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            ConvBlockBHWC(dim=dim, drop_path=dp[i]) for i in range(depth)
        ])
        self.upsample = PatchExpand(dim, dim_scale=2, norm_layer=nn.LayerNorm) if upsample else None

    def forward(self, x):  # x: (B, H, W, C)
        for blk in self.blocks:
            x = blk(x)
        if self.upsample is not None:
            x = self.upsample(x)  # still BHWC
        return x
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        self.layers_up = nn.ModuleList()
        self.concat_back_dim = nn.ModuleList()
        self.decoder_mode = "conv"  # "vss" or "conv"
        print("---decoder mode of Conv ")

        for i_layer in range(self.num_layers):
            # linear reducer after skip concat (kept as-is)
            in_dim = int(self.embed_dim * 2 ** (self.num_layers-1-i_layer))
            concat_linear = nn.Linear(2*in_dim, in_dim) if i_layer > 0 else nn.Identity()
            self.concat_back_dim.append(concat_linear)

            if i_layer == 0:
                # first up step is just PatchExpand (no blocks before it)
                layer_up = PatchExpand(dim=in_dim, dim_scale=2, norm_layer=nn.LayerNorm)
            else:
                depth_here = depths[(self.num_layers-1-i_layer)]
                drop_slice = dpr[sum(depths[:(self.num_layers-1-i_layer)]) : sum(depths[:(self.num_layers-1-i_layer)+1])]

                if self.decoder_mode == "conv":
                    # our new conv-based decoder layer
                    # upsample=True except the last up layer (i_layer < num_layers-1)
                    layer_up = ConvLayer_up(
                        dim=int(self.dims[0] * 2 ** (self.num_layers-1-i_layer)),
                        depth=depth_here,
                        drop_path=drop_slice,
                        upsample=(i_layer < self.num_layers - 1),
                    )
                else:
                    # original VSS-based decoder
                    layer_up = VSSLayer_up(
                        dim=int(self.dims[0] * 2 ** (self.num_layers-1-i_layer)),
                        depth=depth_here,
